...
class StoreResponse(AXMessage):
    ...
    def fromSuccessResponse(cls, success_response, signed=True):
        ...
        @raises AXError: when the AX data cannot be parsed.
        """
        ax_args = success_response.extensionResponse(cls.ns_uri, signed)

        mode = ax_args.get('mode', None)
        error = ax_args.get('error', None)

        try:
            if mode == cls.SUCCESS_MODE:
                self = cls(True)
            elif mode == cls.FAILURE_MODE:
                self = cls(False, error)
            else:
                return None
        except NotAXMessage:
            return None
